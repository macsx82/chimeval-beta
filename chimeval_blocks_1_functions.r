...
CountChimericDNA_block<-function(bedfile,genfile,bamfileDonor,bamindexfileDonor,bamfileAcceptor,bamindexfileAcceptor,bamfileChimer,bamindexfileChimer,CutoffHom,CutoffHet){
   ...
   #########################################################################
    ...
    for(i in 1:dim(CountsDonor)[1]){
        DonorStep1[i,1:2]<-names(CountsDonor[i,order(CountsDonor[i,],decreasing=T)[1:2]])
        ...
        if(DonorStep1[i,"Max"]>(sum(DonorStep1[i,3:4])*CutoffHet[2])&DonorStep1[i,"Max"]<(sum(DonorStep1[i,3:4])*(CutoffHom-0.01))){DonorStep1[i,1:2]<-c("other","other");DonorStep1[i,3:4]<-c(0,0)}
    }
    ...
    for(i in 1:dim(CountsAccetor)[1]){
        ...
        if(AccettorStep1[i,"Max"]>(sum(AccettorStep1[i,3:4])*CutoffHet[2])&AccettorStep1[i,"Max"]<(sum(AccettorStep1[i,3:4])*(CutoffHom-0.01))){AccettorStep1[i,1:2]<-c("other","other");AccettorStep1[i,3:4]<-c(0,0)}
    }

############################## We should need this later
    ...
    for(i in 1:dim(CountsChimers)[1]){
        ...
        if(ChimersStep1[i,"Max"]>(sum(ChimersStep1[i,3:4])*CutoffHet[2])&ChimersStep1[i,"Max"]<(sum(ChimersStep1[i,3:4])*(CutoffHom-0.01))){ChimersStep1[i,1:2]<-c("other","other");ChimersStep1[i,3:4]<-c(0,0)}
    }
...
#Select informative snps for the donor and the acceptor
    ...
    rm(select)
###################################################################################################################
#get total counts from Accettor
# CountsAccettorInformative<-CountsAccetor[rownames(CountsAccetor)%in%rownames(AccettorInformative),]

#TODO:
# write a file with positions for snps informative
...
# if there is only one difference, we nedd to perform a correction on the different genotype
    if(dim(AccettorInformative)[1] >= 3){
        ...
        #we're going to do a correction based on reads content:
        # we need to read each reads, and before counting the bases check if we can correct the read
        # we can correct the read if we find ONLY one difference between chimera read and donor or acceptor genotypes defined before
        #we get the informative acceptor sites and we use them to try a correction
        # then we get the informative donor sites and we use them the same way
        ...
        for(i in 1:dim(AccettorInformative)[1]){
            ...
            # print(ChimersStep1[rownames(AccettorInformative[i,]),])
            if (ChimersStep1[rownames(AccettorInformative[i,]),"B"]=="other"&AccettorInformative[i,"B"]=="other") {
                if(ChimersStep1[rownames(AccettorInformative[i,]),"A"]!=AccettorInformative[i,"A"]){
                    ...
                    to_correct <- rbind(to_correct,CountsChimers[rownames(AccettorInformative[i,]),])
                }
            } else {
        ...
        #write accettor informative table
        write.table(AccettorInformative,file=paste(getwd(),"/Accettor_informative_",ampl,".list",sep=""),quote=F,col.names=F,sep="\t")
        ####################################################################################################################
        #We can do a correction only if we have 1 difference between Chimera and Acceptor
        #TODO: the real correction should be based on ALL READS:
        # 1) we check if the haplotypes are EXACTLY the same as the donor or accettor
            #a) read each read
            # b) build the haplotype that came in that read
            # c) check it against donor and receiver
            # d) tag the read sequence as donor_like, receiver_like, none
            # f) based on the tag, during quantification, find a way to control for the genotyping error
        # 2) We assign the residual reads to the donor or accettor based on if they are more than a certain Threshold
        # 3) We define the threshold as the mean of all accettor reads wrongly quantified
        ####################################################################################################################
...
CountsBasesChimeraCorrection<-function(bedfile,bamfile,bamindexfile,correcttab){
    # bedfile
    # bamfile=bamfileChimer
    # bamindexfile=bamindexfileChimer
    # correcttab=AccettorInformative
    ...
    qseq <- mcols(gal)$seq
    qual <- mcols(gal)$qual
    #here we need to place the correction!!
    # we need to read the chimera data after donor and receiver and use that information to correct the values
    # lets read a single read

    # select snps for correction using the informative ones
    corr_data <- data[which(data$id%in%rownames(correcttab)),]
    # corr_bed <- with(corr_data, GRanges(chr, IRanges(end,width=1), strand="*", score, id=id))
    corr_bed <- with(corr_data, GRanges(chr, IRanges(min(start),max(end)), strand="*", score, id=id))
    
    seqinfo(corr_bed) <- merge(seqinfo(corr_bed), seqinfo(bamfile))
    seqlevels(corr_bed) <- seqlevelsInUse(corr_bed)
    
    corr_which <- as(seqinfo(corr_bed), "GRanges")
    
    corr_flag <- scanBamFlag(isNotPassingQualityControls=NA,isDuplicate=NA)
    corr_param <- ScanBamParam(flag=corr_flag, what=c("seq", "qual"), which=corr_bed)
    
    corr_param_2 <- ScanBamParam(flag=corr_flag, what=c("seq", "qual"), which=corr_which)
    # gal <- readGAlignmentsFromBam(bamfile, param=param)
    corr_gal <- readGAlignments(bamfile, param=corr_param, with.which_label=T)
    corr_gal_2 <- readGAlignments(bamfile, param=corr_param_2, with.which_label=T)
    seqlevels(corr_gal) <- seqlevels(corr_bed)
    seqlevels(corr_gal_2) <- seqlevels(corr_bed)
    qseq_corr <- mcols(corr_gal_2)$seq
    qual_corr <- mcols(corr_gal_2)$qual
    
    for (read in 1:length(qseq_corr)){
        # if (class(try(pileLettersAt(qseq_corr[read,], seqnames(corr_gal), start(corr_gal), cigar(corr_gal), corr_bed), silent=T)) == "try-error"){
        if (class(try(pileLettersAt(qseq_corr[read,], seqnames(corr_gal_2), start(corr_gal_2), cigar(corr_gal_2), corr_bed), silent=T)) == "try-error"){
        if(read%%10000 == 0){
            print(read)
        }
        } else {
            # nucl_piles <- pileLettersAt(qseq_corr[read,], seqnames(corr_gal), start(corr_gal), cigar(corr_gal), corr_bed)
            nucl_piles <- pileLettersAt(qseq_corr[read,], seqnames(corr_gal_2), start(corr_gal_2), cigar(corr_gal_2), corr_bed)
            print(read)
            break
        }
    }
